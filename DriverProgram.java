...
public class DriverProgram extends CreateJson {
		...
		String input=scan.next();
		 ...
		 switch(input)
		 {
			 ...
			 if(opt==4)
				...
				create(key,c+scan.next().replaceAll("\\s","").replaceFirst("\\{",","),opt);
						 else
				        create(key,scan.next().replaceAll("\\s", ""),opt);
		 	...
		 	case "4":
		 	{
		 		System.out.println("|thank you welcom|");
		 		return;
		 	}
